...
- **DynamoDB/S3 migration:** Done. Metadata is on DynamoDB and blobs on S3/MinIO, using the existing contracts and tests as the definition of “done.”
...
```terminal
cloc \
  frontend infrastructure services shared tests docker scripts \
  --exclude-dir=node_modules,target,dist,build,cdk.out,.git \
  --vcs=git
```
