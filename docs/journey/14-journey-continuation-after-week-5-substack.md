...
I went back to the repo, ran the tests, and tried to build everything. Turns out a lot of the services didn’t even compile. The AI had been reporting success while tests were failing and calling the rest “pre-existing” problems. In one case it had even removed functionality so the Docker build would succeed. So we had a green build and less working code. The project manager in me wept. (Not exactly progress.)

First order of business: make everything compile. Then figure out what was actually missing or broken. The glamour never ends. Only then did “completion percentage” start to mean anything.

## What I Learned (Weeks 2–5)

**Nothing is “done” until it builds and tests run.** Completion percentages from the AI were meaningless until we had green builds and real test results. Obvious in hindsight. Painful in practice. (Story of my life.)

**AI assistants overclaim.** Cursor kept saying things were done while tests were failing. It had a real attachment to the phrase “pre-existing” for anything it didn’t fix. The lesson: treat every “done” from an agent as a hypothesis. Run the build and the test suite yourself.
...
## One of the Fails: The Home Page

Week 3: the new Angular app was “up.” I opened the new home page next to the old one. My exact reaction: *Holy macaroni. What are we looking at?*

**Old home page (legacy):**

![Home page (old)](07-jorney-week-3-home-page-old.png)

**New “modernized” home page:**

![Home page (new)](08-jorney-week-3-home-page-new.png)

Very interesting. The new stack was running, but “migrated” clearly meant “we have a shell,” not “we have a replacement.” That’s when we ran a proper gap analysis and got to ~22% complete (design system, layout, notifications, categories, drag-and-drop—all still missing). Lesson: don’t declare the front-end done until you’ve compared it to the old UI and measured what’s missing. Also: for actually migrating the visuals, Codex and Claude were far more useful than Cursor’s Auto mode, which failed spectacularly on that job.
...
- **DynamoDB/S3** — Done. Metadata is on DynamoDB and blobs on S3/MinIO, using the existing contracts and tests as the definition of “done.”
...
- **Measure progress with criteria** — No more “92%” without a definition. Use: “all services compile,” “integration suite green,” “front-end phase N complete.” (DynamoDB/S3 migration: done.)
...
One number that *is* real: we’re at **124,652 total lines of code** since Week 1 — more than the original repo. Java leads (~40K), then TypeScript (~28K), JSON (~20K), YAML, shell, Markdown, and the rest; [full breakdown by language](10-journey-week-5-loc.png). A lot of that is the extra testing we added once we stopped trusting “92% done.” So: more code, more tests, and a clearer picture of what’s actually left. I’ll take it.
